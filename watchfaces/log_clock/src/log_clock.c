...
static void handle_second_tick(struct tm *tick_time, TimeUnits units_changed) {
  ...
  static char log_text[] = "123456";//6
    ...
    strftime(s_log_text, sizeof(s_log_text), "%S   LOG ", tick_time);
    //insert log
    memmove(s_log_text + 9, log_text, 4);//4 digit after dp
    v = (int)(j*100)%100;//converted
    snprintf(log_text, sizeof(log_text), "%02d", v);
    memmove(s_log_text + 2, log_text, 2);//2 digit after dp
