...
static struct tm *sw;//stopwatch
static struct tm lap;

void save_clock() {
  persist_write_data(SW_STORE, &sw, sizeof(sw));
...
static struct tm * sub_mod(struct tm *now, struct tm *then) {
  static struct tm buf;
  buf.tm_hour = (now->tm_hour - then->tm_hour)%24;
  buf.tm_min = (now->tm_min - then->tm_min)%60;
  buf.tm_sec = (now->tm_sec - then->tm_sec)%60;
  return &buf;
}
...
void tick_clock(struct tm *tick_time) {
  clear();
  if((sw_butt&4)==1) { 
    sw = tick_time;//stopwatch not running a delta
    sw_butt ^= 4;//reset
  }
  ...
  case 1: draw_dp(0b100, 3);//date
    ...
    break;
  case 2: draw_dp(0b101110, 2);//stopwatch
    struct tm * show = sub_mod(tick_time, ((sw_butt&2)==0)?sw:&lap);
    tock(show);
    break;
  default: break;
  }
...
void load_clock() {
  ...
  if(persist_exists(SW_STORE)) {
    persist_read_data(SW_STORE, t, sizeof(sw));
      ...
      if(mode==2) {
        ...
        case BUTTON_ID_UP://lap/reset
          if((sw_butt&1)==0) {//not running
            if((sw_butt&2)==1) //showing lap
              sw_butt ^= 2;
            else //showing counter
              sw_butt = 5;//reset
          } else { //running
            sw_butt ^= 2;
            memcpy(&lap, sw, sizeof(lap));//get lap buffer
          }
          break;
